...
function buildWeb(domain, expoPublicReplId) {
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      if (code === 0) {
        ...
        // Post-process: inject PWA manifest link + SW registration into index.html
...
async function main() {
  ...
  // Build web bundle first — it manages its own Metro instance
  await buildWeb(domain, expoPublicReplId);

  // Now start Metro for the iOS/Android Expo Go bundles
